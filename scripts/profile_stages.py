...
def main():
    ...
    def run(users, mode, iters):
        output = subprocess.check_output(
            [args.binary, str(iters), mode, str(users)], text=True
        )
        return (float(re.search(r'ns_per_iter=([\d.]+)', output)[1]),
                int(re.search(r'bytes=(\d+)', output)[1]))
    ...
    for users in args.sizes:
        for mode in modes:
            ...
            row = dict(users=users, bytes=sizes[users], mode=mode,
